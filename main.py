...
class TwitterBrowser:
    ...
    async def post_thread(self, content):
        """Tweet thread olarak içerik paylaşma"""
        try:
            # İçeriği liste veya string olarak işleme
            if isinstance(content, list):
                ...
                try:
                    # Liste veya string olmayan içeriği stringe çevirmeye çalış
                    thread_content = [str(content)]
                    self.logger.info(f"⚠️ İçerik tipi dönüştürüldü: {type(content)} -> str")
                except Exception as e:
                    self.logger.error(f"❌ Geçersiz içerik tipi: {type(content)}")
                    return False

            self.logger.info(f"📝 Thread içeriği: {len(thread_content)} tweet")
            
            # Her bir tweet için
            for i, item in enumerate(thread_content):
                if not isinstance(item, str):
                    # String olmayan elemanları stringe çevir
                    tweet_text = str(item).strip()
                    self.logger.info(f"🔄 Tweet {i+1} string'e çevrildi")
                else:
                    tweet_text = item.strip()
                
                # Karakter limiti kontrolü
                if len(tweet_text) > 280:
                    tweet_text = tweet_text[:277] + "..."
                    self.logger.info(f"✂️ Tweet {i+1} kısaltıldı (>280 karakter)")
                
                # Tweet yazma alanını aç (ilk tweet için)
                if i == 0:
                    try:
                        await self.page.goto("https://x.com/compose/tweet", timeout=30000)
                        await asyncio.sleep(3)
                        self.logger.info("📝 Tweet compose sayfası açıldı")
                    except Exception as e:
                        self.logger.error(f"❌ Tweet compose sayfası açılamadı: {e}")
                        return False
                
                # Tweet yazma alanını bul
                compose_element = None
                for selector in ["div[data-testid='tweetTextarea_0']", "div[contenteditable='true']", "div[role='textbox']"]:
                    try:
                        compose_element = await self.page.query_selector(selector)
                        if compose_element:
                            self.logger.info(f"✅ Tweet compose alanı bulundu: {selector}")
                            break
                    except Exception:
                        continue
                        
                if not compose_element:
                    self.logger.error("❌ Tweet compose alanı bulunamadı!")
                    return False
                
                # İçeriği yaz - birkaç farklı yöntem dene
                ...
                try:
                    ...
                    self.logger.info("✅ Tweet içeriği fill ile yazıldı")
                except Exception as e:
                    self.logger.warning(f"⚠️ Fill ile yazma başarısız: {e}")
                ...
                if not content_written:
                    try:
                        ...
                        await asyncio.sleep(1)
                        await self.page.keyboard.type(tweet_text)
                        await asyncio.sleep(2)
                        content_written = True
                        self.logger.info("✅ Tweet içeriği keyboard.type ile yazıldı")
                    except Exception as e:
                        self.logger.warning(f"⚠️ Type ile yazma başarısız: {e}")
                
                # Yöntem 3: evaluate - bu sefer f-string kullanmadan
                if not content_written:
                    try:
                        # Çift tırnakları escape et
                        safe_text = tweet_text.replace('"', '\\"')
                        js_code = 'document.querySelector("' + selector + '").innerText = "' + safe_text + '"'
                        await self.page.evaluate(js_code)
                        await asyncio.sleep(2)
                        content_written = True
                        self.logger.info("✅ Tweet içeriği evaluate ile yazıldı")
                    except Exception as e:
                        self.logger.warning(f"⚠️ Evaluate ile yazma başarısız: {e}")
                
                if not content_written:
                    self.logger.error("❌ Tweet içeriği yazılamadı!")
                    return False
                
                # Gönder butonuna tıkla
                send_success = False
                for send_selector in ["[data-testid='tweetButton']", "div[data-testid='tweetButtonInline']", 
                                     "button[data-testid='tweetButton']", "[role='button']:has-text('Tweet')"]:
                    try:
                        send_button = await self.page.query_selector(send_selector)
                        if send_button:
                            await send_button.click()
                            await asyncio.sleep(5)  # Tweet gönderildikten sonra bekle
                            send_success = True
                            self.logger.info(f"✅ Tweet {i+1} gönderildi!")
                            break
                    except Exception as e:
                        self.logger.warning(f"⚠️ {send_selector} ile gönderme başarısız: {e}")
                
                # Eğer normal butonlarla başarısız olursa, klavye kısayolu dene
                if not send_success:
                    try:
                        await self.page.keyboard.press("Control+Enter")
                        await asyncio.sleep(3)
                        self.logger.info(f"✅ Tweet {i+1} klavye kısayolu ile gönderildi!")
                        send_success = True
                    except Exception as e:
                        self.logger.error(f"❌ Klavye kısayolu ile gönderme başarısız: {e}")
                        return False
                
                # Thread'in bir sonraki tweet'i için + butonunu tıkla
                if i < len(thread_content) - 1:
                    try:
                        # Thread + butonu
                        plus_button = await self.page.query_selector("div[aria-label='Add post']")
                        if plus_button:
                            await plus_button.click()
                            await asyncio.sleep(2)
                            self.logger.info("➕ Thread'e yeni tweet ekleniyor")
                    except Exception as e:
                        self.logger.error(f"❌ Thread'e yeni tweet eklenemedi: {e}")
                        return False
            
            return True
        except Exception as e:
            self.logger.error(f"❌ Thread gönderme hatası: {e}")
            return False
